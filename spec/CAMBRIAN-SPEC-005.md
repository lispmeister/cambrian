...
> **Implementation status:** MAP-Elites is **deferred to Stage 2+**. Stage 1 uses single-objective Bayesian Optimization (`supervisor/bo_loop.py`) which maximizes `viability_rate` directly. The MAP-Elites design below is retained for Stage 2 planning — it has no code yet.
...
> **Implementation status:** The Meta-Monitor is **deferred to Stage 2+**. It has no code yet. The triggers and neutral drift logic below are retained for Stage 2 planning.
