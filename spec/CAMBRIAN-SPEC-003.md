...
A generation lives or dies at the container level. Promote means merge the branch and tag it. Revert means destroy the container and discard the branch. There is no partial promotion, no rollback-after-promote. The decision is final and recorded.
...
- **Spec** — This document. The genome. Defines what Prime is. Input to code generation.
...
- `POST /spawn` is asynchronous. It creates the container, starts the Test Rig, and returns immediately. Prime polls `GET /stats` or waits for the generation to appear in `GET /versions` with a terminal `outcome`.
...
| `CAMBRIAN_SPEC_PATH`  | MAY      | `./spec/CAMBRIAN-SPEC-003.md` | Path to the spec file |
...
```
1. Prime starts in container. Reads spec from /workspace/spec/CAMBRIAN-SPEC-003.md.
...
4. LLM returns 8 files: src/prime.py, src/api.py, src/generate.py,
   ...
   manifest.json, spec/CAMBRIAN-SPEC-003.md.
...
**Done when:** A hand-crafted artifact with a valid `manifest.json` can be spawned, tested, promoted, and rolled back through the Supervisor API. The Test Rig writes a correct viability report.
...
The promoted Prime reproduces itself.

**Adds:** Nothing new — this phase validates that Phase 1 produced a Prime capable of reproduction.

**Done when:** Gen-1 Prime reads the spec, calls an LLM, produces gen-2, gen-2 passes the Test Rig, and gen-2 Prime can itself produce a gen-3 that passes the Test Rig. This is the M1 acceptance criterion.
...
- Gen-1 Prime (bootstrap-produced) passes the Test Rig
- Gen-1 Prime generates gen-2
- Gen-2 passes the Test Rig
- Gen-2 Prime generates gen-3
- Gen-3 passes the Test Rig

If gen-3 is viable and was produced by gen-2 which was produced by gen-1, M1 is complete.

## References

- [CAMBRIAN-SPEC-002](CAMBRIAN-SPEC-002.md) — Previous iteration of this spec
...
**Fitness** is an environment concern, not part of the genome. For M1, viability is binary: the Test Rig passes or it doesn't. When selection between viable organisms becomes relevant (M2+), fitness criteria will be defined in a separate environment spec — measured by the environment, not self-reported by the organism.
